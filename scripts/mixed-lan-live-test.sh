...
octet=$((0x$octet_hex))
network="smacx-mixed-live-$suffix"
runner="smacx-mixed-runner-$suffix"
...
# No parent interface means Docker supplies an isolated dummy parent. The
# runner is attached to the same macvlan as every game worker, matching the
# shipped Control Center topology without emitting packets onto the host LAN.
...
docker run --rm --name "$runner" --network "$network" \
    ...
    python3 scripts/mixed_lan_live_test.py
