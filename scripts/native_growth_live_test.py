...
def main():
    ...
    docker = DockerClient()
    with tempfile.TemporaryDirectory(prefix='smacx-growth-') as tmp:
        ...
        worker = None
        try:
            source = manager.validate_game_source(os.environ['SMACX_TEST_GAME_SOURCE'], display_name='Growth native comparison')
            runtime = manager.ensure_bundled_runtime()
            control.store.ensure_agent('agent-support-test', 'Support test')
            match = control.create_solo_match('Growth native comparison', 'agent-support-test', faction_id=1)
            ...
            for _ in range(180):
                ...
                if snap['interaction']['kind'] != 'turn':
                    ...
                    time.sleep(.25); continue
                result = call('test_managed_action_fixture', phase='diagnostics_growth_comparison')
                assert result.get('ok'), result
                growth = result['before']['growth']
                assert growth['nutrient_threshold'] == result['threshold']
                assert growth['population_room'] and not growth['ordinary_growth_inhibited']
                assert result['first_population'] == 2 and result['first_stock'] == result['threshold'], result
                assert result['second_population'] == 3, result
                assert result['inhibited_before']['growth']['ordinary_growth_inhibited'], result
                assert result['inhibited_population'] == 2, result
                assert result['boom_before']['growth']['population_boom_conditions_met'], result
                assert result['boom_population'] == 3, result
                assert not result['cap_before']['growth']['population_room'], result
                assert result['capped_population'] == result['cap_before']['population'], result
                print(json.dumps({'passed': True, 'native_comparison': result}), flush=True)
                return
