...
def frame(*args):
 ...
 if mode=='intro': choices['c']['meaning']='Acknowledge this information-only faction introduction.'
 ...
 information=[{'pending':'not verified'}]
 if mode=='production-upgrade': information=[{'kind':'information','event':'future_production_upgraded',
   'previous_prototype_name':'Scout Patrol','replacement_prototype_name':'Synthmetal Sentinel',
 ...
 return {'ok':True,'decision_id':d,'identity':{'match_id':'m','session_id':'s','revision':revision},
         'focus':{'kind':'interaction','popup_label':'PRODUPGRADE' if mode=='production-upgrade' else 'SIMULYOU'},
...
for mode in ('normal','intro','production-upgrade','generic','choice','capture-fail','unchanged','reject','queued'):
 events.clear();count=0
 result=m.smac_decision()
 if mode in ('normal','intro','production-upgrade'): assert events==['capture','execute']*4 and count==5
