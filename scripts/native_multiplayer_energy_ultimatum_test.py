#!/usr/bin/env python3
"""Two-client saved-state demand replay. Requires the private turn-15 ultimatum fixture.
...
SAVE_SHA256 = '01898589b5bf7a07d47248238c0af6f39bb95571c03f58fe3deffe553c6fc0db'
...
def exercise(manager, ids, response):
    ...
    while time.monotonic() < deadline and target is None:
        for n, instance in enumerate(ids):
            ...
            if kind == 'turn' and n == 0 and moves < 1:
                unit, tile = [(0, 1893)][moves]
    ...
    counter = command(ids[0], target, 'respond_to_diplomatic_offer', response='counter')
    assert counter.get('ok'), counter
    deadline = time.monotonic() + 30
    while time.monotonic() < deadline:
        snap = call(ids[0], 'semantic_snapshot')['snapshot']
        if snap['interaction'].get('popup_label') == 'WEASELOUT':
            target = call(ids[0], 'semantic_choices', kind='interaction')
            break
        time.sleep(.2)
    else:
        raise AssertionError(('counter did not reach ultimatum', snap))
    assert not command(ids[0], target, 'respond_to_diplomatic_offer', response='counter').get('ok')
