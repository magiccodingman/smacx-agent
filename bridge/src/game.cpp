...
int __cdecl top_menu(int flag) {
    debug("top_menu %d\n", flag);
    // The semantic agent may request a configured Quick Start before the main
    // menu is constructed. This follows the game's native non-interactive
    // setup path (map_menu/config_game with flag 1) and performs no UI input.
    static bool agent_autostart_consumed = false;
    char agent_autostart[8] = {};
    if (!flag && !agent_autostart_consumed
    && GetEnvironmentVariableA("SMACX_AGENT_AUTOSTART", agent_autostart, sizeof(agent_autostart))
    && !strcmp(agent_autostart, "1")) {
        agent_autostart_consumed = true;
        prefs_load(0);
        prefs_use();
        char value[32] = {};
        if (GetEnvironmentVariableA("SMACX_AGENT_DIFFICULTY", value, sizeof(value))) {
            DefaultPrefs->difficulty = clamp(atoi(value), 0, MaxDiffNum - 1);
        }
        ...
        if (GetEnvironmentVariableA("SMACX_AGENT_FACTION_ID", value, sizeof(value))) {
            DefaultPrefs->faction_id = clamp(atoi(value), 1, MaxPlayerNum - 1);
        }
        memset(value, 0, sizeof(value));
        if (GetEnvironmentVariableA("SMACX_AGENT_BLIND_RESEARCH", value, sizeof(value))) {
            if (!strcmp(value, "1")) {
        ...
        *GameState = (*GameState & ~(STATE_RAND_FAC_LEADER_PERSONALITIES
        ...
        memset(value, 0, sizeof(value));
        if (!GetEnvironmentVariableA("SMACX_AGENT_NARRATIVE_UI", value, sizeof(value))
        || strcmp(value, "1")) {
            *GamePreferences |= PREF_AV_INTERLUDES_DISABLED;
            *GameMorePreferences |= MPREF_AV_MONUMENTS_DISABLED;
        }
        memset(value, 0, sizeof(value));
        if (!GetEnvironmentVariableA("SMACX_AGENT_TUTORIAL_UI", value, sizeof(value))
        || strcmp(value, "1")) {
...
CONF_FINAL:
        ...
        if (!*MultiplayerActive && !(flag & 2)) {
            // Semantic autostart must provide initial research before
            // planetfall(), because the original game otherwise enters a
            // synchronous picker that blocks the simulation thread.  The
            // input is the ordinary player-visible research direction (or an
            // explicitly selected exact tech when blind research is off).
            char agent_autostart[8] = {};
            if (GetEnvironmentVariableA("SMACX_AGENT_AUTOSTART", agent_autostart,
                sizeof(agent_autostart)) && !strcmp(agent_autostart, "1")) {
                Faction& player = Factions[MapWin->cOwner];
                if (player.tech_research_id < 0 && (*GameRules & RULES_BLIND_RESEARCH)) {
                    char priority_text[16] = {};
                    int priority = TCAT_TECH;
                    if (GetEnvironmentVariableA("SMACX_AGENT_INITIAL_RESEARCH_PRIORITY",
                        priority_text, sizeof(priority_text))) {
                        priority = clamp(atoi(priority_text), int(TCAT_GROWTH), int(TCAT_POWER));
                    }
                    player.AI_growth = priority == TCAT_GROWTH;
                    player.AI_tech = priority == TCAT_TECH;
                    player.AI_wealth = priority == TCAT_WEALTH;
                    player.AI_power = priority == TCAT_POWER;
                    player.tech_research_id = mod_tech_ai(MapWin->cOwner);
                    debug("agent_setup_research blind faction=%d priority=%d tech=%d %s\n",
                        MapWin->cOwner, priority, player.tech_research_id,
                        tech_str(player.tech_research_id));
                } else if (player.tech_research_id < 0) {
                    char tech_text[16] = {};
                    if (GetEnvironmentVariableA("SMACX_AGENT_INITIAL_TECH_ID",
                        tech_text, sizeof(tech_text))) {
                        int tech_id = atoi(tech_text);
                        if (tech_id >= 0 && tech_id < MaxTechnologyNum
                        && tech_avail(tech_id, MapWin->cOwner)) {
                            player.tech_research_id = tech_id;
                            debug("agent_setup_research directed faction=%d tech=%d %s\n",
                                MapWin->cOwner, tech_id, tech_str(tech_id));
                        }
                    }
                }
                // Direct semantic preselection bypasses mod_tech_pick's tail,
                // which normally initializes the revised per-target research
                // cost. Without this, mod_tech_rate deliberately returns its
                // 9999 sentinel and an otherwise valid game cannot advance.
                if (player.tech_research_id >= 0 && conf.revised_tech_cost
                && !*MultiplayerActive && player.tech_cost <= 0) {
                    player.tech_cost = tech_alt_cost(player.tech_research_id, MapWin->cOwner);
                    debug("agent_setup_research cost faction=%d tech=%d cost=%d\n",
                        MapWin->cOwner, player.tech_research_id, player.tech_cost);
                }
            }
            char tutorial_ui[8] = {};
            if (!GetEnvironmentVariableA("SMACX_AGENT_TUTORIAL_UI", tutorial_ui,
                sizeof(tutorial_ui)) || strcmp(tutorial_ui, "1")) {
                *GamePreferences &= ~PREF_BSC_TUTORIAL_MSGS;
                AlphaIniPrefs->preferences &= ~PREF_BSC_TUTORIAL_MSGS;
            }
            // prefs_use/setup_game can overwrite the pre-menu values. Apply
            // the spectator presentation policy again after game setup so
            // later base/unit event checks do not open blocking interludes.
            char narrative_ui[8] = {};
            if (!GetEnvironmentVariableA("SMACX_AGENT_NARRATIVE_UI", narrative_ui,
                sizeof(narrative_ui)) || strcmp(narrative_ui, "1")) {
                *GamePreferences |= PREF_AV_INTERLUDES_DISABLED;
                *GamePreferences &= ~PREF_AV_SECRET_PROJECT_MOVIES;
                *GameMorePreferences |= MPREF_AV_MONUMENTS_DISABLED;
            }
            planetfall(MapWin->cOwner);
            // Deterministic regression seam for the contained Xephyr suite.
            // It is inert in normal launches and is never set by the MCP.
            // Diplomacy otherwise depends on random map proximity or pods,
            // which makes exercising its native blocking event loop flaky.
            char test_mode[8] = {};
            char test_contact[16] = {};
            if (GetEnvironmentVariableA("SMACX_AGENT_TEST_MODE", test_mode, sizeof(test_mode))
            && !strcmp(test_mode, "1")
            && GetEnvironmentVariableA("SMACX_AGENT_TEST_CONTACT", test_contact,
                sizeof(test_contact))) {
                int other = atoi(test_contact);
                if (other >= 1 && other < MaxPlayerNum && other != MapWin->cOwner
                && is_alive(other)) {
                    treaty_on(MapWin->cOwner, other, DIPLO_COMMLINK);
                    debug("agent_test_contact faction=%d other=%d\n", MapWin->cOwner, other);
                }
            }
            char test_council[8] = {};
            if (GetEnvironmentVariableA("SMACX_AGENT_TEST_MODE", test_mode, sizeof(test_mode))
            && !strcmp(test_mode, "1")
            && GetEnvironmentVariableA("SMACX_AGENT_TEST_COUNCIL", test_council,
                sizeof(test_council)) && !strcmp(test_council, "1")) {
                for (int other = 1; other < MaxPlayerNum; ++other) {
                    if (other == MapWin->cOwner || !is_alive(other)) continue;
                    treaty_on(MapWin->cOwner, other, DIPLO_COMMLINK);
                }
                debug("agent_test_council faction=%d all_contacts=1\n", MapWin->cOwner);
                char test_incoming_council[8] = {};
                if (GetEnvironmentVariableA("SMACX_AGENT_TEST_INCOMING_COUNCIL",
                    test_incoming_council, sizeof(test_incoming_council))
                && !strcmp(test_incoming_council, "1")) {
                    for (int first = 1; first < MaxPlayerNum; ++first) {
                        if (!is_alive(first)) continue;
                        Factions[first].council_call_turn = -999;
                        for (int second = 1; second < MaxPlayerNum; ++second) {
                            if (second != first && is_alive(second)) {
                                treaty_on(first, second, DIPLO_COMMLINK);
                                // Keep the contained ballot regression stable long
                                // enough for the human-facing Council windows to
                                // be observed. The random starting positions can
                                // otherwise eliminate the test faction during the
                                // first AI phase and auto-resolve its ballot.
                                treaty_on(first, second, DIPLO_TREATY);
                                treaty_on(first, second, DIPLO_PACT);
                            }
                        }
                    }
                    debug("agent_test_incoming_council mutual_pacts=1 cooldowns_reset=1\n");
                }
                char test_council_policy[8] = {};
                if (GetEnvironmentVariableA("SMACX_AGENT_TEST_COUNCIL_POLICY",
                    test_council_policy, sizeof(test_council_policy))
                && !strcmp(test_council_policy, "1")) {
                    *GovernorFaction = MapWin->cOwner;
                    int tech_id = Proposal[PROP_GLOBAL_TRADE_PACT].preq_tech;
                    if (tech_id >= 0 && tech_id < MaxTechnologyNum) {
                        TechOwners[tech_id] |= 1 << MapWin->cOwner;
                    }
                    debug("agent_test_council_policy faction=%d proposal=%d tech=%d\n",
                        MapWin->cOwner, PROP_GLOBAL_TRADE_PACT, tech_id);
                }
            }
            char test_workshop[8] = {};
            if (GetEnvironmentVariableA("SMACX_AGENT_TEST_MODE", test_mode, sizeof(test_mode))
            && !strcmp(test_mode, "1")
            && GetEnvironmentVariableA("SMACX_AGENT_TEST_WORKSHOP", test_workshop,
                sizeof(test_workshop)) && !strcmp(test_workshop, "1")) {
                const int component_techs[] = {
                    Chassis[CHS_SPEEDER].preq_tech,
                    Weapon[WPN_PARTICLE_IMPACTOR].preq_tech,
                    Armor[ARM_SYNTHMETAL_ARMOR].preq_tech,
                    Ability[ABL_ID_TRANCE].preq_tech,
                    Ability[ABL_ID_DROP_POD].preq_tech,
                };
                ...
                Factions[MapWin->cOwner].energy_credits = 1000;
                debug("agent_test_workshop faction=%d energy=%d\n",
                    MapWin->cOwner, Factions[MapWin->cOwner].energy_credits);
            }
            char test_transport[8] = {};
            if (GetEnvironmentVariableA("SMACX_AGENT_TEST_MODE", test_mode, sizeof(test_mode))
            && !strcmp(test_mode, "1")
            && GetEnvironmentVariableA("SMACX_AGENT_TEST_TRANSPORT", test_transport,
                sizeof(test_transport)) && !strcmp(test_transport, "1")) {
                const int component_techs[] = {
                    Chassis[CHS_FOIL].preq_tech,
                    Weapon[WPN_TROOP_TRANSPORT].preq_tech,
                };
                for (int tech_id : component_techs) {
                    if (tech_id >= 0 && tech_id < MaxTechnologyNum) {
                        TechOwners[tech_id] |= 1 << MapWin->cOwner;
                ...
                debug("agent_test_transport_components faction=%d\n", MapWin->cOwner);
            }
        }
        return 0;
    }
...
int __cdecl end_of_game(int flag) {
    ...
    case VIC_LOST_CAPTURE:
        if (*GamePreferences & PREF_AV_SECRET_PROJECT_MOVIES) {
            if (!MFactions[player_id].is_alien()) {
                agent_begin_endgame_presentation("victory_movie");
                mod_amovie_project(MFactions[player_id].is_leader_female ? "losewoman" : "loseman");
                agent_end_endgame_presentation("victory_movie");
            } else {
                agent_begin_endgame_presentation("victory_movie");
    ...
    case VIC_ALIEN_SOLO:
        agent_begin_endgame_presentation("victory_interlude");
        interlude(32, 0, 4, 0);
        agent_end_endgame_presentation("victory_interlude");
        setup_parser(victor_id);
    ...
    || *GameVictoryType == VIC_TRANSCEND_LOSS) {
        agent_begin_endgame_presentation("credits");
        show_credits();
        agent_end_endgame_presentation("credits");
    }
    agent_begin_endgame_presentation("score_report");
    report_score(1);
    agent_end_endgame_presentation("score_report");
    if (!(*GameState & STATE_IS_SCENARIO)) {
        agent_begin_endgame_presentation("quayle_rating");
        quayle(player_id);
        agent_end_endgame_presentation("quayle_rating");
        agent_begin_endgame_presentation("hall_of_fame");
        hall_of_fame(1);
        agent_end_endgame_presentation("hall_of_fame");
    }
    agent_begin_endgame_presentation("replay");
    show_replay();
    agent_end_endgame_presentation("replay");
