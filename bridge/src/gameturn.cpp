...
void __cdecl turn_upkeep() {
    ...
    *CurrentMissionYear = game_year(++(*CurrentTurn));
    ...
    || *CurrentTurn >= 7) {
        ...
        if (aliens_arrive) {
            // Preserve the engine script identity while this synchronous
            // notification is open so the semantic agent can acknowledge it
            // without pixels or simulated input.
            agent_popp(ScriptFile, "ALIENSARRIVE", 0, "al_land_sm.pcx", 0);
            char semantic_agent[8] = {};
            char narrative_ui[8] = {};
            bool semantic_autostart =
                GetEnvironmentVariableA("SMACX_AGENT_AUTOSTART", semantic_agent, sizeof(semantic_agent))
                && !strcmp(semantic_agent, "1");
            bool show_narrative =
                GetEnvironmentVariableA("SMACX_AGENT_NARRATIVE_UI", narrative_ui, sizeof(narrative_ui))
                && !strcmp(narrative_ui, "1");
            if (!semantic_autostart || show_narrative) {
                interlude(21, 0, 1, 0);
            }
        }
    }
