...
volatile int agent_mp_local_research_selections = 0;
volatile int agent_mp_remote_research_waits = 0;
volatile int agent_mp_blind_research_picks = 0;
volatile int agent_mp_async_research_syncs = 0;
volatile int agent_mp_last_research_faction = -1;
volatile int agent_mp_last_research_priority = -1;
...
int __cdecl mod_tech_selection(int faction_id) {
    ...
    if (*MultiplayerActive && is_human(faction_id) && faction_id != *CurrentPlayerFaction) {
        ++agent_mp_remote_research_waits;
        bool done = false;
        ...
        if (*MultiplayerActive && is_human(faction_id)) {
            ...
            char semantic_agent[8] = {};
            if (GetEnvironmentVariableA("SMACX_AGENT_ENABLE", semantic_agent,
                sizeof(semantic_agent)) && !strcmp(semantic_agent, "1")) {
                // Synchronization command 0x0A copies this faction's
                // tech_research_id through the native NetDaemon.  The stock
                // routine clears the field immediately and enters a nested
                // network wait for the echoed value.  Two independently
                // controlled semantic clients can both become trapped in
                // those nested waits during planetfall.  Keep the selected
                // value in place and let the ordinary event loop receive the
                // same native synchronization packet asynchronously.  This
                // changes no payload or authority: every peer still learns
                // the selection through DirectPlay, while the exact blind
                // technology remains absent from the semantic API.
                ++agent_mp_async_research_syncs;
                debug("agent_research_sync_async faction=%d\n", faction_id);
                return tech_id;
            }
...
int __cdecl mod_tech_pick(int faction_id, int flag, int other_faction_id, const char* label) {
    Faction& plr = Factions[faction_id];
    // A normal tech picker is synchronous and blocks the simulation thread.
    // For semantic autostart, accept the same pre-game research decision via
    // environment before the picker opens.  This is deliberately consumed
    // once, applies only to the human faction's initial research, and never
    // exposes or accepts a hidden exact technology in blind-research mode.
    static bool agent_initial_research_consumed = false;
    char autostart[8] = {};
    char semantic_agent[8] = {};
    if (!flag && other_faction_id < 0 && *CurrentTurn == 0 && is_human(faction_id)
    && (*GameRules & RULES_BLIND_RESEARCH)
    && GetEnvironmentVariableA("SMACX_AGENT_ENABLE", semantic_agent, sizeof(semantic_agent))
    && !strcmp(semantic_agent, "1")) {
        ...
        plr.AI_growth = priority == TCAT_GROWTH;
        plr.AI_tech = priority == TCAT_TECH;
        plr.AI_wealth = priority == TCAT_WEALTH;
        plr.AI_power = priority == TCAT_POWER;
        ++agent_mp_blind_research_picks;
        agent_mp_last_research_faction = faction_id;
        agent_mp_last_research_priority = priority;
        int tech_id = mod_tech_ai(faction_id);
        if (tech_id >= 0 && revised_tech_cost()) {
            plr.tech_cost = tech_alt_cost(tech_id, faction_id);
        }
        debug("agent_blind_research faction=%d priority=%d tech=%d %s\n",
            faction_id, priority, tech_id, tech_str(tech_id));
        return tech_id;
    }
    if (!agent_initial_research_consumed && !flag && other_faction_id < 0
    && *CurrentTurn == 0 && is_human(faction_id)
    && GetEnvironmentVariableA("SMACX_AGENT_AUTOSTART", autostart, sizeof(autostart))
    && !strcmp(autostart, "1")) {
        agent_initial_research_consumed = true;
        if (*GameRules & RULES_BLIND_RESEARCH) {
            ...
            plr.AI_power = priority == TCAT_POWER;
            int tech_id = mod_tech_ai(faction_id);
            debug("agent_initial_research blind faction=%d priority=%d tech=%d %s\n",
                faction_id, priority, tech_id, tech_str(tech_id));
            return tech_id;
        }
        char tech_text[16] = {};
        if (GetEnvironmentVariableA("SMACX_AGENT_INITIAL_TECH_ID", tech_text, sizeof(tech_text))) {
            int tech_id = atoi(tech_text);
            if (tech_id >= 0 && tech_id < MaxTechnologyNum && tech_avail(tech_id, faction_id)) {
                debug("agent_initial_research directed faction=%d tech=%d %s\n",
                    faction_id, tech_id, tech_str(tech_id));
                return tech_id;
            }
        }
        // Invalid directed preselection: preserve the native picker rather
        // than silently choosing a technology the player did not request.
    }
    ...
    if (faction_id == MapWin->cOwner || *GameState & STATE_OMNISCIENT_VIEW) {
        ...
        if (faction_id == MapWin->cOwner) {
            if (*MultiplayerActive && (*DiploWinState || *dword_7492CC || *ControlWaitLoop)) {
                ...
                agent_new_technology_presentation(
                    NetTechWin, tech_id, faction_id_2);
            }
