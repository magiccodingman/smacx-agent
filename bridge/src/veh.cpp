...
/*
Upgrade exactly one vehicle and consume its remaining turn, matching the
human-player path through Console::upgrade. Candidate legality and the
single-player guard are enforced by the semantic bridge before this helper is
called. Returns the charged energy cost, or -1 if the vehicle/cost is invalid.
*/
int __cdecl single_unit_upgrade(int veh_id, int new_unit_id) {
    if (veh_id < 0 || veh_id >= *VehCount || new_unit_id < 0
    || new_unit_id >= MaxProtoNum) return -1;
    VEH* veh = &Vehs[veh_id];
    int faction_id = veh->faction_id;
    int old_unit_id = veh->unit_id;
    if (faction_id <= 0 || old_unit_id < 0 || old_unit_id >= MaxProtoNum
    || new_unit_id == old_unit_id
    || new_unit_id / MaxProtoFactionNum != faction_id) return -1;
    int cost = 10 * mod_upgrade_cost(faction_id, new_unit_id, old_unit_id);
    if (cost < 0 || Factions[faction_id].energy_credits < cost) return -1;
    veh_upgrade(veh, new_unit_id, old_unit_id, cost);
    veh_skip(veh_id);
    if (new_unit_id >= MaxProtoFactionNum) {
