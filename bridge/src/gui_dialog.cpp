...
namespace {
std::string current_agent_popup_label;
std::string last_started_agent_popup_label;
BasePop* last_started_agent_popup = NULL;
uint64_t last_started_agent_popup_generation = 0;
std::string last_started_agent_popup_strings[8];
int last_started_agent_popup_numbers[8] = {};

class AgentPopupScope {
    std::string previous;
public:
    explicit AgentPopupScope(const char* label) : previous(current_agent_popup_label) {
        current_agent_popup_label = label ? label : "";
    }
    ~AgentPopupScope() { current_agent_popup_label = previous; }
};
}

const char* agent_popup_label() {
    if (!*WinModalState && !*PopupDialogState) return "";
    if (!current_agent_popup_label.empty()) return current_agent_popup_label.c_str();
    // A different modal layer can remain active after the most recently
...
uint64_t agent_popup_generation() {
    return last_started_agent_popup_generation;
}

void agent_popup_started(const char* label, BasePop* popup) {
    ++last_started_agent_popup_generation;
    last_started_agent_popup_label = label ? label : "";
    last_started_agent_popup = popup;
    // Capture the substitutions at construction time. The engine reuses these
    // global buffers while a modal is open, so later reads are not stable enough
    // for semantic context such as the completed base/item names.
    for (int index = 0; index < 8; ++index) {
        last_started_agent_popup_strings[index] = ParseStrBuffer[index].str;
        last_started_agent_popup_numbers[index] = ParseNumTable[index];
    }
}

const char* agent_popup_parse_string(int index) {
    return index >= 0 && index < 8
        ? last_started_agent_popup_strings[index].c_str() : "";
}

int agent_popup_parse_number(int index) {
    return index >= 0 && index < 8 ? last_started_agent_popup_numbers[index] : 0;
}

int agent_popp(const char* filename, const char* label, int flags,
    const char* imagefile, fp_none fn) {
    AgentPopupScope scope(label);
    return popp(filename, label, flags, imagefile, fn);
}
