...
int __thiscall Win_is_visible(Win* This) {
    if (!This) return 0;
    // Some real BasePop objects use a small numeric parent sentinel (for
    // example 2) instead of a Win object.  The child remains visible, but the
    // sentinel must never itself be dereferenced as a window.
    if (reinterpret_cast<uintptr_t>(This) < 0x10000) return 1;
...
{
    agent_bridge_start_once(hwnd);
    if (agent_bridge_handle_message(hwnd, msg)) {
        return 0;
    }
...
{
    // Record the engine script identifier at the common popup construction
    // point. This covers engine-originated dialogs as well as Thinker calls
    // and lets the semantic bridge classify them without reading pixels.
    agent_popup_started(label, This);
