...
static bool test_force_captive_leader_penetration(int action_id) {
    if (action_id != PRB_FREE_CAPTURED_FACTION_LEADER) return false;
    char test_mode[8] = {};
    char test_leader[8] = {};
    return GetEnvironmentVariableA("SMACX_AGENT_TEST_MODE", test_mode, sizeof(test_mode))
        && !strcmp(test_mode, "1")
        && GetEnvironmentVariableA("SMACX_AGENT_TEST_PROBE_LEADER", test_leader,
            sizeof(test_leader))
        && !strcmp(test_leader, "1");
}
...
int probe_unit_mind_control_cost(int probe_veh_id, int target_veh_id) {
    if (probe_veh_id < 0 || probe_veh_id >= *VehCount
    || target_veh_id < 0 || target_veh_id >= *VehCount
    || !Vehs[probe_veh_id].is_probe()
    || Vehs[probe_veh_id].faction_id == Vehs[target_veh_id].faction_id
    || mod_stack_check(target_veh_id, 1, -1, -1, -1) > 1) return -1;
    VEH& probe_veh = Vehs[probe_veh_id];
    VEH& target_veh = Vehs[target_veh_id];
    int probe_faction_id = probe_veh.faction_id;
    int target_faction_id = target_veh.faction_id;
    if (!target_faction_id || target_veh.is_native_unit()
    || (target_veh.triad() == TRIAD_AIR
        && !has_abil(probe_veh.unit_id, ABL_AIR_SUPERIORITY))
    || (MFactions[target_faction_id].rule_flags & RFLAG_MINDCONTROL)) return -1;
    int distance_factor = vulnerable(target_faction_id, target_veh.x, target_veh.y);
    if (target_veh.home_base_id >= 0
    && has_fac_built(FAC_PUNISHMENT_SPHERE, target_veh.home_base_id)) distance_factor /= 2;
    int cost = Units[target_veh.unit_id].cost
        * ((Factions[target_faction_id].energy_credits + 800) / (distance_factor + 2));
    for (int i = 0; i < MFactions[probe_faction_id].faction_bonus_count; ++i) {
        if (MFactions[probe_faction_id].faction_bonus_id[i] == RULE_PROBECOST) {
            cost = MFactions[probe_faction_id].faction_bonus_val1[i] * cost / 100;
        }
    }
    if (has_project(FAC_NETHACK_TERMINUS, probe_faction_id)) cost = 3 * cost / 4;
    int probe_rating = Factions[target_faction_id].SE_probe;
    bool enhanced_probe = has_abil(probe_veh.unit_id, ABL_ALGO_ENHANCEMENT);
    if (probe_rating <= -2) cost /= 2;
    else if (probe_rating == -1) cost += cost / -4;
    else if (probe_rating == 1) cost += cost / 2;
    else if (probe_rating == 2 || enhanced_probe) cost *= 2;
    else if (probe_rating > 2) return -1;
    if (has_abil(target_veh.unit_id, ABL_POLY_ENCRYPTION)) cost *= 2;
    if (target_veh.plan() != PLAN_COLONY && target_veh.plan() != PLAN_TERRAFORM) cost /= 2;
    return max(0, cost);
}
...
int __cdecl probe(int veh_id, int tgt_base_id, int tgt_veh_id, int toggle) {
    ...
    *ProbeHasAlgoEnhancement = 0;
    ...
    if (tgt_base_id < 0) {
        ...
        action_id = ((*VehAttackFlags & 4)
            && (Vehs[veh_id].probe_action & 15) == PRB_FREE_CAPTURED_FACTION_LEADER)
            ? PRB_FREE_CAPTURED_FACTION_LEADER : Vehs[veh_id].probe_action & 7;
    } else {
...
MOV_SABOTAGE:
    ...
    int act_val;
    // Bit 2 is reserved by the semantic bridge for a staged native probe
    // decision.  Primary mission selection is still preseeded (bit 0 stays
    // clear), but the real post-entry target and warning dialogs remain live
    // so their choices can be exposed without inspecting the target base
    // before the game would reveal them to a human player.
    semantic_staged_choice = (*VehAttackFlags & 4) != 0;
    ...
    if (veh_fc_id == MapWin->cOwner && is_human(veh_fc_id) && *VehAttackFlags & 1) {
        ...
        if (semantic_staged_choice && act_val
        && !mod_success_rates(2, mod_morale_veh(veh_id, 1, 0), 1, tgt_base_id)) {
            // Match the native ADVVIRUS branch: when targeted sabotage has a
            // zero success rate, widespread random sabotage is the only
            // available continuation and no target menu is revealed.
            act_val = 0;
        }
    }
    if (!act_val) {
    ...
    prb_diff = 1;
    if (veh_fc_id != MapWin->cOwner || !is_human(veh_fc_id)
    || (!(*VehAttackFlags & 1) && !semantic_staged_choice)) {
...
MOV_DEFEND:
    ...
    if (test_force_captive_leader_penetration(action_id)) rnd_val = prb_diff;
    if (rnd_val >= prb_diff) {
    ...
    if (prb_state < 0) {
        if (!is_human(-prb_state)) {
            ...
            if (*MultiplayerActive) {
                ...
                agent_set_probe_excuse_context(veh_fc_id, -prb_state, action_id, true, true);
                if (!plr_alien) {
                    choice = popp_2("PACTFRAMEEXCUSE", "capture_sm.pcx", 0);
                } else {
                    choice = popp_2("PACTFRAMEEXCUSE", "al_cap_sm.pcx", 0);
                }
                agent_clear_probe_excuse_context();
                if (choice) {
                    pact_ends(veh_fc_id, -prb_state);
                    Factions[-prb_state].diplo_spoke[veh_fc_id] = *CurrentTurn;
                }
            } else {
                agent_set_probe_excuse_context(veh_fc_id, -prb_state, action_id, true, false);
                if (!plr_alien) {
    ...
    if (is_human(tgt_fc_id)) {
        ...
        if (*MultiplayerActive) {
            ...
            if (has_treaty(tgt_fc_id, veh_fc_id, DIPLO_PACT)) {
                ...
                agent_set_probe_excuse_context(veh_fc_id, tgt_fc_id, action_id, false, true);
                int excuse_choice = BasePop_exec_2(&cur_popup);
                agent_clear_probe_excuse_context();
                if (excuse_choice) {
                ...
                agent_set_probe_excuse_context(veh_fc_id, tgt_fc_id, action_id, false, false);
                int excuse_choice = BasePop_exec_2(&cur_popup);
                agent_clear_probe_excuse_context();
                if (excuse_choice) {
                    if (action_id >= PRB_MIND_CONTROL_CITY) {
