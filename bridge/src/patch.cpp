...
bool patch_setup(Config* cf) {
    ...
    *(int32_t*)GetSystemMetricsImport = (int32_t)ModGetSystemMetrics;
    ...
    const int network_wait_calls[] = {
        0x530369, 0x530421, 0x53056A, 0x530738, 0x530828,
        0x5309EC, 0x530B45, 0x530C0E, 0x530F3A, 0x531242,
        0x531393, 0x53151D, 0x53165E, 0x532441, 0x5327B1,
    };
    for (size_t index = 0;
    index < sizeof(network_wait_calls) / sizeof(network_wait_calls[0]); ++index) {
        write_call(network_wait_calls[index], (int)agent_network_wait_task);
    }
    write_call(0x5EE247, (int)agent_modal_wait_task); // Win::exec wait boundary
    write_call(0x5BBA22, (int)agent_new_technology_presentation); // tech_achieved
    ...
    write_call(0x537AD5, (int)agent_chat_display); // NetDaemon::process_message
