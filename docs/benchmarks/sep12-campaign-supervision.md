...
## Turn-35 movement rejection and park reconciliation

AI10 progressed to turn 35 before stopping on its existing repetition circuit. The provider selected the same Former move from location 2251 to location 2291 three times. The first two native calls returned a terminal rejection with the same observed origin; each fresh frame nevertheless advertised the exact target as `safe_local_move`. The third submission was stopped before dispatch. Captured requests contained the alternatives, so this was not missing provider context; it was a false-positive movement catalog plus insufficient post-rejection suppression.

The native safe-move catalog now mirrors the stock `order_veh` constraint for an ordinary uncloaked land unit crossing between two hostile movement-ZOC tiles. A private two-client replay of the bridge-paired turn-35 checkpoint removes targets 2291, 2331 and 2250 while preserving legal targets 2211, 2252 and 2171 plus finish and terraform choices. See [sanitized evidence](turn35-movement-rejection.json). Independently, a definitive attempted rejection with a confirmed unchanged position withholds only that exact semantic choice until meaningful native progress; it does not infer permanent illegality or hide alternatives. The actual MCP adapter regression passes.

AI9 also demonstrated a transport ambiguity after checkpoint capture: stopping a sovereign committed, but the portal received an ended HTTP response and treated parking as failed. Parking now reconciles that ambiguous response against durable harness-run status. It proceeds only when the exact run is no longer active; queued, starting, running or restarting states still fail closed. Portal state tests cover all six states. Live park/recovery acceptance remains pending after deployment.

## Turn-35 local modal ownership

After movement recovery, AI10's Spartan client restored into an information-only `INTRO` popup while faction 1 owned the underlying ordinary turn. The low-thinking sovereign interpreted `current_faction_id: 1` as ownership of that popup and yielded `WAITING` three times without calling `smac_decision`. The third unchanged clean yield correctly opened `incident-7b18d5b0bc4f4b48957bf5b4d82ab723`; the circuit and quarantine policy were not relaxed.
