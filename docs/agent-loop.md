...
`match_id` is the durable namespace for one playthrough. `session_id` changes whenever the game process is relaunched. Match-scoped notes belong under `$HOME/Documents/ai/SidMeiers/games/<match_id>/`; the launcher creates `match.json` there. Its `sessions` ledger records each process start, save origin when loaded, and clean stop, while `status` identifies the current lifecycle state. Named saves live under `runtime/game/saves/agent/<match_id>/`. Loading preserves `match_id`, creates a new `session_id`, and invalidates every old guard and engine object ID.
