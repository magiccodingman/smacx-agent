...
| --- | --- | --- | --- | --- | --- | --- | --- |
| Invalid decision recovery | fresh semantic snapshot and choice enumeration after unknown/expired/consumed decision or invalid choice | same sovereign, perspective, briefing and turn gates as `smac_decision`; current native protocol is separate from lagging projected order summaries | rejection remains visible; automatic read supplies `recovery.frame`; consumed source frames are explicitly superseded on the provider wire | original failure plus new opaque frame in execution receipt; replacement choice source and prohibited old IDs are explicit; frame-local ordinal choice handles reduce exact-copy failure without exposing native meaning | rejected action is never replayed; paired decision lookup, private payload, native revision and single-use guards remain | four-failure circuit unchanged; refresh cannot reset budget; no cross-session frame substitution; wait/handoff retained | `decision_recovery_test.py`, opaque execution, runtime-context and actual Hermes sanitizer/HTTP-wire contracts; live replay diagnosis in [multiplayer scouting evidence](benchmarks/multiplayer-scouting-semantics.md) and earlier repair in [turn-15 evidence](benchmarks/consumed-choice-replay-turn15.md) |
| Map and mutable terrain | paged tiles, semantic snapshot | owned/visible plus native remembered items; Unity Survey terrain/altitude only when native-entitled; unseen resources/ownership remain unknown | changed known location; coastline/road/pod changes re-anchor | world area/route/reachability/render | opaque Former, road/tube, airdrop and guarded adjacent visible supply-pod choices | current/stale native fields; pod benefit/danger remains unknown until native resolution; snapshot rebuilt on rollback | topology, fair-play, terrain-destruction and strategic fixtures; compiled eligibility matrix plus two-client native pod removal |
...
| Expansion/base-site mechanics | target-specific bounded native legality/yield/radius receipt plus known topology | receipt requires a currently visible nominated location; hidden rejection reasons excluded | queried site dependency changes invalidate receipt/query | world compare; nominated alternatives are recommended before consequential founding | founding remains an opaque current-unit choice carrying current spacing, nearest-known-base and known-radius-overlap evidence | legal land/sea founding, current yields, radius visibility, known overlap, resources, ownership, connectivity, transport, hostile proximity and frontier exposure are facts; no best site; legal does not mean strategically good | native receipt source contract, materially different candidate fixture and provider-safe settlement-choice semanticization |
...
| Own units and support | paged units plus rich unit adapter | owned full only | ready/order/location/damage/lifecycle changes | world forces/logistics/relation | opaque unit actions and persistent orders, including two-client native Auto Explore and activation | monotonic semantic refs survive native compaction and verified checkpoint/process recovery; nested native support cost is projected while Clean/no-support units are excluded | unit lifecycle/orders/ready refs, private identity restart, support-shape and persistent-orders fixtures; exact two-client Explore state convergence |
...
| Diplomacy and chat | faction relations, native dialogs, semantic chat | contacted parties; message recipient/channel only | critical chat, relation/commitment changes | world intel/global; chat/memory recall | exact offers, responses, native and group chat | speech is reported; mechanics current; deduped message UID | diplomacy, LAN diplomacy, chat and communication contracts; [turn-7 multiplayer energy-demand evidence](benchmarks/multiplayer-energy-demand.md) |
...
| Production, citizens and governors | Native production population JSON and managed transport-error propagation validated; the original paired checkpoint restored both seats and is `restore_tested`; recovery now carries explicit doctrine approval, waits for all sovereign runs, and suppresses overlapping automatic restore during peer reconnection ([evidence](benchmarks/production-json-recovery.md)). Owned queue, governor and citizen fields visible through world base queries | Set/append/remove queue, governor and worker/specialist changes execute and appear in world state | Existing guarded journal actions retained; completion/progression attention belongs to checkpoint 3 |
...
### Guarded multiplayer pod and settlement-evidence checkpoint

The managed action path now distinguishes intentional `collect_supply_pod`
from ordinary safe movement, revalidates current native eligibility before the
stock LAN order, waits through native unwind, and reports only observed pod
removal. A real two-client Thinker fixture verified identical peer state after
the pod effect. Founding choices now carry current spacing and known-radius
overlap evidence, while doctrine states that opaque reference spelling has no
spatial meaning and directs consequential settlement comparison through
`smac_world`. Compiled negative gates, managed receipt/journal/schema tests,
provider-safe semanticization, doctrine fixtures, and the native command are
...
### Turn 57: expiry recovery, persisted correction and Sensor completion

Actual managed trace rejected a 334.207-second-old choice under the 300-second lease without native dispatch. The sovereign obtained a fresh frame and completed End Turn about 25 seconds later, with native turn56→57 receipt and canonical action journal. Corrected monitor plan was present in subsequent actual provider runtime envelopes. Current direct-sight tile836 changed from road/rolling/vehicle at56 to road/rolling/sensor/vehicle at57, following recorded Sensor work2→4→6; the Former now has no active task and automation remains enabled. This closes live expiry-feedback consumption and this Sensor completion chain, not mineral improvement, general recovery correctness, or expansion competence. See `benchmarks/astra-followthrough57.json`.

Turn57 citizen-path follow-up: accepted conversion and assignment are independently confirmed by current observations440→441 (Doctor removed; minerals3→4, food4→5). The final +0 mineral/+1 food balance equals the pre-conversion balance. Sovereign corrected its M1-for-M1 gain claim before assignment. This is successful native command execution and a strategically unnecessary reassignment, not improved economics. See `benchmarks/astra-citizen-reconsideration57.json`.

Turn58 scope-recovery acceptance: actual managed records show a mistyped perspective rejected, followed by sovereign-corrected identity/revision, confirmed canonical goal/plan writes, and guarded native57→58. This proves this rejection was consumed without bypass or operator retry. Former remains automation-enabled with no active terraform order at58; new work is not inferred. See `benchmarks/astra-scope-recovery58.json`.

Scope-error feedback repair: repeated turn58 truncated-session writes motivated explicit persistence-not-started and fresh-decision guidance. All four identity mismatches remain rejected before writing; corrected retry retains the native revision guard. New scope feedback and existing guarded-memory contract tests pass. Deployment and live feedback consumption remain pending. See `benchmarks/astra-memory-scope-feedback59.json`.
...
Threat attribution62: actual provider runtime carries observed Worm HP reductions to0 and subsequent contact removals, plus surviving contact877. Sovereign noticed this evidence. Attribution of the kills to its alert Scouts remains unverified; the delivered contact deltas do not name the attacker. Preserve that distinction rather than promoting a plausible explanation to native causal evidence. See `benchmarks/astra-worm-attribution62.json`.

Farm effect62: current/current observations457→465 within the native turn transition show tile835 gaining Farm and headquarters food intake5→6/surplus1→2. Prior Farm work2→6 plus this feature/economic change closes this completion-and-food-benefit chain. Mineral surplus stays0; no colony progress or exact future growth date is implied. See `benchmarks/astra-farm-effect62.json`.

Deliberation recurrence62: two completed calls/14,063 tokens triggered the existing365-second stop while a subsequent provider request was about78 seconds old. The preceding request took205 seconds. Native/collectors are quarantined and diagnostics preserved. Prior prompt guidance is not a sufficient remedy; bounded in-flight handling/budget feedback require review, with no read-as-progress or unbounded timeout relaxation. See `benchmarks/astra-deliberation-recurrence62.json`.

Request-state telemetry checkpoint: bounded sovereign-only run/request phases now distinguish submitted, headers, transport completion, failure and early close. Existing telemetry helper reads only matching-run state; reads do not reset semantic progress and stop policy is unchanged. Controlled HTTPX streaming/failure/stale-completion/specialist tests and existing audit/watchdog contracts pass. Deployment, live observation and bounded policy repair remain pending; campaign stays quarantined.
...
## LAN HTTPS streaming checkpoint (2026-09-08)
...
## Sovereign event wait checkpoint (2026-09-08)
...
| Provider content liveness | AI - 7's interruption was observed during active reasoning. Trusted SSE content is represented by a private request-scoped watermark; a bounded drain distinguishes silence and excessive generation from native progress. AI - 10 then exposed a sequential-request false stop: a new active request could not inherit the already-fixed deliberation deadline. Controlled streaming/terminal/identity, same-run request handoff, and supervisor reconciliation tests preserve the independent gameplay clock without resetting the absolute bound. Memory schema guidance separates journal events from observation cursors and actors from agent/seat IDs. The installed image passed, and recovered AI - 10 advanced from the failed turn 59 boundary through turn 60 to turn 61 with two active sovereigns and no incident. See [acceptance](benchmarks/provider-stream-liveness.md). |
...
| Adjacent movement rejection recovery | Live turn-35 false-positive safe move → two terminal native rejections with unchanged position → third attempt stopped by existing circuit → stock movement-ZOC gate represented in native catalog → private two-client checkpoint replay excludes the three blocked targets while preserving legal/finish/terraform choices → exact rejected semantic choice withheld only for unchanged state. Public redeployment and sustained reuse pending. |
| Park stop-response ambiguity | Verified checkpoint captured → sovereign stop response lost after possible commit → durable exact-run reconciliation → only inactive terminal state permits park continuation; active states remain blocked → six-state portal contract passes. Live park retry pending. |
| Client-local modal ownership | Live turn-35 `INTRO` with foreign ordinary-turn owner → three bounded clean yields and safe incident → `current_faction_id` explicitly scoped to ordinary turn actions while `phase=interaction` is represented as a mandatory client-local blocker → fixed doctrine and progress-aware resume prompt require a fresh decision → runtime/prompt/continuation/wait contracts pass → verified paired recovery replays the same state → reviewed notification drain dismisses the local blocker → Spartan receives authoritative `phase=wait` while Peacekeeper receives `phase=turn`; both campaigns are observed active with healthy worker/MCP pairs and no active incidents. First hourly sample advances AI9 27→35 and AI10 35→38 with no incident; healthy samples 1/4, so sustained stability remains pending. [Evidence](benchmarks/turn35-local-modal-ownership.json), [deployment read-back](benchmarks/sep12-campaign-supervision.md#turn-35-local-modal-ownership). |
| Production-upgrade local notice | Live turn-37 `PRODUPGRADE` → exact blocking label and zero native alternatives observed → managed catalog represents a sole acknowledgement → provider copies fresh handles → multiplayer allowlist rejects the advertised action three times → repetition circuit preserves both peers and paired checkpoint → stock Script distinguishes completed `PRODUPGRADE` from consequential `ASKPRODUPGRADE` → exact reviewed label and old/replacement prototype context added → unknown multiplayer one-button labels no longer advertise an unexecutable acknowledgement. Compiled predicate, notification, decision, action-path and doctrine integration contracts plus native link pass; the reviewed engine fingerprint is registered. Live replay captured `Synthmetal Garrison` → `Plasma Garrison`, executed one guarded acknowledgement, retained healthy peers, and continued native progress. Four-hour stability pending. [Evidence](benchmarks/turn37-production-upgrade-notice.json). |
| Concurrent recovery storage isolation | Live AI10 turn-43 recovery → retained failure names AI9 `.git/gc.pid` → storage accounting identified as the non-authoritative cross-match traversal → `.git` excluded and file sizes sampled atomically per entry with disappearance tolerated → journal authority remains strict → doctrine-start recovery failures now close maintenance instead of hanging. Control-plane and focused portal contracts pass. Sequential live recovery restored AI9 turn 36 and AI10 turn 43 on the new prepared image, rebuilt every Graphiti perspective, restarted every sovereign, and did not reproduce the cross-match failure. [Evidence](benchmarks/concurrent-recovery-storage-race.json). |

| Multiplayer scouting and threat semantics checkpoint | Acceptance chain and limits |
| --- | --- |
| Persistent LAN exploration | Live AI9/AI10 observations showed ready Scouts repeatedly skipped or held while multiplayer exposed only one-tile movement → LAN unit catalog now represents `auto_explore_unit` for current ready combat units → guarded fail-closed validation → authoritative native Explore state → `synch_veh` publication → two independent DirectPlay clients agree on assignment and cancellation → exploring units are activate-only. The engine still chooses paths and applies fog, costs, encounters, and waking; strategic use and exploration quality remain model behavior pending a fresh campaign. |
| Base-defense role accuracy | Current owned co-location → broad present-unit list plus current combat-role classification → `observed_defender_count` counts only combat-capable defenders → noncombat and stale/unknown roles remain separately queryable → request-only nearby-defense context carries the distinction. ETA is explicitly reachability rather than an attack forecast; HP decline has unknown cause without a separate current native event. Focused spatial/runtime, movement, counterfactual, fair-play, managed-action, doctrine, bridge-build, and two-client tests pass. Provider behavioral calibration remains pending. |
| Provider-reliable guarded selection | AI9 turn 47 copied every fresh decision handle but invented four UUID-shaped choice handles → random per-enumeration handles replaced by bounded frame-local ordinals → private decision binding, exact pair lookup, expiry, scope, revision, single-use, native validation, journal authority, and the four-failure circuit remain intact → controlled recovery and opaque-execution tests reject invented, stale, replayed, consumed, and cross-scope handles → installed source check passes and supported recovery advances AI9 47→48 without incident. Fresh autonomous choice copying remains under observation. |
...
| --- | --- |
| Start/end communications blackout presentation | Live AI9 turn-70 gap with native duration 12 → exact `gameturn.cpp` ordering proves `SunspotDuration` changes before each presentation → `SUNSPOTS` and `NOMORESPOTS` represented as exact reviewed information with current duration and active state → guarded multiplayer acknowledgement only → exact incident-save replay reaches both notices → both DirectPlay clients dismiss each local modal and agree on duration 12 at start and 0 at end → installed control/worker image → verified 5/5 AI9 recovery from turn 69 → both production sovereigns capture the structured start event and execute one `choice-01` acknowledgement → new turn-70 checkpoint save-verifies with no incident. Compiled label, reviewed fingerprint, bridge-build, action-path, recovery, opaque-choice, notification, and doctrine contracts pass. Sustained progression and a production ending-notice recurrence remain pending. |

See [sunspot notice acceptance](benchmarks/sunspots-multiplayer-notice.md).

| Graphiti rebuild recovery checkpoint | Acceptance chain and limits |
| --- | --- |
| Provider drop during checkpoint-memory restore | AI10 turn-81 restore → one perspective rebuilt while the second stayed queued behind an unhealthy projector → restart claims the durable request → backing provider closes the socket and leaves the rebuild `running` without completion → per-request and whole-rebuild bounds added → projector restart requeues interrupted `running` work with explicit provenance → required perspective rebuilds drain before optional background projection → sovereign admission continues to require every perspective rebuild → focused scheduling, requeue, timeout, liveness, failure-visibility, journal-authority, and production-adapter capture contracts pass → public image `6456944b…` completes the interrupted rebuild → explicit verified turn-81 recovery rebuilds both perspectives → the priority build immediately claims the second queued perspective → both rebuilds complete without error → operator health reports `observed_active`, two live/two active sovereigns, two healthy native/MCP pairs, no verification error, and no incident. AI9 advances from turn 70 to turn 76 during the repair. Sustained AI10 progression and a deliberately forced production timeout remain pending. |

See [Graphiti rebuild recovery acceptance](benchmarks/graphiti-rebuild-recovery.md).

| Persistent-directive live candidate checkpoint | Acceptance chain and limits |
| --- | --- |
| Compression-deferred supervision | Exact candidate `e5800a1` → AI10 turn 84→85 → Hermes provider ceiling rejection and concurrent compression soft defer → three zero-exit retries misclassified as sovereign no-progress → incident packet retained with diagnostics SHA256 `486321a4…` → AI9 operator-frozen at turn 94 → host-owned defer marker and bounded retry added → genuine no-action and token-spending stall guards unchanged → focused continuation/strict-prompt and 24-suite directive validation pass. Rebuilt deployment, recovery, clean paired measurement, directive adoption, provider-free directive slices, performance and sustained stability remain pending. |
| Base mind-control notice | Live AI9 turn-102 `MINDCONTROL1` → exact `probe.cpp` path shows role-specific `MINDCONTROL0/1/2`, one ignored popup result, and ownership transfer continuing after dismissal → prior unknown-label multiplayer guard safely advertises no action → diagnostic bundle SHA256 `51c2b82d…` retains three synchronized saves → exact three-label reviewed acknowledgement plus attributed attacker/former-owner/base context and pending-effect status → malformed/unknown labels remain rejected. Exact-label predicate/source and managed-path validation pass with all 24 directive/affected regression suites; native cross-build, post-dismissal live ownership verification and clean benchmark restart remain pending. |
| Clean pinned directive adoption | Exact candidate `ece37d63f49163ae5cc84db792cd79798e382ac3` → verified restore boundaries AI9 turn 101 and AI10 turn 91 → clean observation through turns 108/99 → recoverable opportunity proxy populated with 22 unique ready actor/revision/unit observations across 12 AI9 units and 30 across 10 AI10 units → directive vocabulary present in 86/109 runtime-context and selection diagnostics → zero assignments for both profiles → zero deterministic execution, completion, interruption, and provider-inside-slice events. The executor's live value remains untested because neither sovereign adopted the path; further passive duration is not marked as acceptance. Both campaigns finish parked with verified checkpoints, stopped workers/MCP, and zero active sovereigns. See `docs/benchmarks/unit-directives.md`. |
