...
| 4 | Final provider request, runtime envelope and omission audit | Actual Hermes process capture equals controlled receiving provider in four profiles; journal selection inventory and GC metrics | Deployed wire exposed stale saved system on resume; actual two-invocation Hermes regression now verifies replacement plus history continuity. Fresh AI - 4 wire matches approved hash; resumed AI - 4 wire also matches and retains handoff conversation; real Hermes length-continuation now passes exclusive lease enforcement and GC-stable episode identity; real receiving-provider tests now verify direct15/9 schemas across resume, direct MCP guard receipt and GC for both namespaces; longer-run deployed verification remains |
...
| 9 | Ready-unit focus preserves management access | Nonmandatory focus metadata; native management effects with units present; typed review | Live last-ready Auto Explore closed turns before intended management; offered choices now expose existing conservative closure risk, opaque/intent guard contracts pass; deployed provider receipt pending |
...
| 16 | Campaign diagnostics download | Scope, compressed/corrupt-tail, immutable-prefix, retention and constrained helper contracts; portal 72/72 including authorization | Deployed archive passes ZIP integrity with journal/history/explicit historical gaps; desktop/390px layout checked. Browser download reports ERR_BLOCKED_BY_CLIENT, so delivery remains unverified; live ZIP passes in 2.242s/855,386 bytes with one in-flight request explicitly reported; browser delivery remains pending |
| 17 | Minimal prompt/tool education | Doctrine/schema contracts; explicit missing-schema retrieval education; exact operational prompt 1,349 Qwen tokens (+25 measured); live prefix reuse evidence | Explicit recompile succeeded in profile but saved Hermes prompt reached wire; request-time canonical replacement now passes real resume test. Fresh deployed hash verified in AI - 4; new approved schema-education hash verified on resumed wire; schema retrieval observed, continued behavior monitoring remains |
