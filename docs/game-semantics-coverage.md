...
| Foreign contacts | visible native units plus bounded semantic movement/damage/destruction feed | current visibility only; identity retained only across mechanically proven continuously visible movement and retired through fog | contact appeared/moved/damaged/destroyed/lost is critical | world forces/intel/changes | combat/probe choices only when natively legal | opaque visible-episode identity; estimated lost envelope | fair-play honeytokens, advance/retreat, rendezvous, fog pursuit, combat tests |
...
| **Partial — Pact/infiltration intelligence** | contacted faction adapter and entitled foreign economy/research/orbital enrichments | exact Pact/infiltration channel, never generic foreign fields; not every stock foreign-report screen is projected yet | material relation/intel changes | world intel | native diplomatic choices | field-level entitlement provenance attached; timeline-scoped | compiled native adapter, entitlement adversarial test and diplomacy tests |
...
| **Partial — Secret Projects and victory races** | owned/known production, completed-project public registry and public victory state | own production plus completed public projects and legitimately observed reports; unobserved rival production races remain unknown | project/global changes re-anchor | world base/global/changes | production, Artifact acceleration, victory actions | source channel retained; unknown rivals stay unknown | native global adapter, project/Artifact/endgame fixtures |
...
| **Partial — Orbital systems** | owned orbital totals and infiltrated foreign totals | owned state plus exact infiltration entitlement; no generic foreign orbital disclosure | orbital/global change | world global | only native-returned legal production/actions | owned/infiltration field provenance; rollback-derived | native global adapter and entitlement fixtures |
...
until a real adapter populates them. Unity Survey and current Planetary
Governor status are read directly from native rule/governor state; scenario
rules, owned orbitals, completed public Secret Projects, and public victory
state are also native-backed. Synthetic entitlement envelopes alone never
upgrade a row to complete.
