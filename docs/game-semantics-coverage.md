...
| --- | --- | --- | --- |
| 1 | Named causal calls and receipts | Actual sovereign/MCP/native records link arguments, opaque choices, results, latency and publication receipts; pre-dispatch failures are captured. See [findings](gameplay-diagnostics-findings.md). | Continue correlation checks during the acceptance game; historical uncaptured calls remain explicitly unknown. |
...
| 8 | Production and citizen actions | Native retool/queue/citizen comparisons pass. Actual turn32 wire retained production and citizen answers; sovereign-selected hurry changed22→30 minerals and88→69 credits. Native turn33 produced the Colony Pod. [Evidence](benchmarks/gameplay-complementary-result-delivery.json). | The requery followed an operational repair notice. Subsequent production requery and Former selection are observed; native+AI recovery34 retains the completed Pod. Citizen tile/yield omission and unexplained two-step reassignment are now repaired with guarded adapter tests; deployed; expanded native comparison and autonomous allocation use pending. |
...
| 13 | Spatial provenance | Route/area/mechanical query arguments, results and epistemic evidence are retained. Historical eyeballed estimates had no retained route query. | Attribute future suspect distances/threat estimates using actual query evidence; missing historical evidence cannot be recreated. |
| 14 | Failures and operational recovery | Pre-MCP rejection, argument validation, watcher allocation, stable failure taxonomy and failure-origin capture pass. Popup use-after-lifetime repair passes matched native replay, autonomous Social Psych and active-dialog/LAN regression. [Final-cut catch-up](benchmarks/gameplay-contact-feed-alignment.json) addresses the turn32 recurrence. | Separate invalid_tile_id origin remains unreproduced with telemetry deployed. Continue live final-cut recurrence/performance and recovery checks. |
...
| 19 | Integrated acceptance game | Reviewable checkpoint commits and evidence are on draft PR55. Verified checkpoints and native+AI restores preserve the campaign. Autonomous gameplay reached33 and produced its first Colony Pod after repairs. | Full game is unfinished. Continue monitoring, repair, PR updates and deployment; never merge automatically. |
