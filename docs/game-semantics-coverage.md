...
| 14 | Tool/runtime failure audit | Pre-MCP executor and earlier unknown-name rejection capture, cross-thread emitted-call correlation, authenticated lifecycle error capture, typed metrics and unmatched provider requests | Deployed failures and operational incident export; restricted Hermes checkpoint read permission repaired; one intermittent cold-start cause unresolved |
