...
| Production, citizens and governors | Native production population JSON and managed transport-error propagation validated ([evidence](benchmarks/production-json-recovery.md)); campaign restore pending. Owned queue, governor and citizen fields visible through world base queries | Set/append/remove queue, governor and worker/specialist changes execute and appear in world state | Existing guarded journal actions retained; completion/progression attention belongs to checkpoint 3 |
