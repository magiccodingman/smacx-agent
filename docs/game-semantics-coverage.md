...
| --- | --- | --- | --- | --- | --- | --- | --- |
| Invalid decision recovery | fresh semantic snapshot and choice enumeration after unknown/expired/consumed decision or invalid choice | same sovereign, perspective, briefing and turn gates as `smac_decision` | rejection remains visible; automatic read supplies `recovery.frame` | original failure plus new opaque frame in execution receipt | rejected action is never replayed; sovereign selects anew; native revision/single-use guards remain | four-failure circuit unchanged; refresh cannot reset budget; no cross-session frame substitution; wait/handoff retained | `decision_recovery_test.py`, opaque/failure-budget contracts, actual Hermes sanitizer + controlled HTTP wire; live campaign recovery tracked in [evidence](benchmarks/decision-recovery-turn8.md) |
