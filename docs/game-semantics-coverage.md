...
| --- | --- | --- | --- |
| 1 | Named causal calls and receipts | Actual sovereign/MCP/native records link arguments, opaque choices, results, latency and publication receipts; pre-dispatch failures are captured. Compact human menu rendering now prevents repeated move rows from hiding later action families (887-character43-choice regression); deployment pending. See [findings](gameplay-diagnostics-findings.md). | Continue correlation checks during the acceptance game; historical uncaptured calls remain explicitly unknown. |
...
| 14 | Failures and operational recovery | Pre-MCP rejection, argument validation, watcher allocation, stable failure taxonomy and failure-origin capture pass. Popup use-after-lifetime repair passes matched native replay, autonomous Social Psych and active-dialog/LAN regression. [Final-cut catch-up](benchmarks/gameplay-contact-feed-alignment.json) addresses the turn32 recurrence. | Turn41 exposed read-only WAL checkpoint capture failure; private staging preserves source/journals and scoped filtering in helper regressions, with deployed checkpoint/recovery pending. Separate invalid_tile_id origin remains unreproduced with telemetry deployed. Continue live final-cut recurrence/performance and recovery checks. |
...
| 19 | Integrated acceptance game | Reviewable checkpoint commits and evidence are on draft PR55. Verified checkpoints and native+AI restores preserve the campaign. Autonomous gameplay reached38 and produced its first Colony Pod after repairs; verified38 restores on the movement-receipt repair and advances to39 with two native bases. | Full game is unfinished. Continue monitoring, repair, PR updates and deployment; never merge automatically. |
