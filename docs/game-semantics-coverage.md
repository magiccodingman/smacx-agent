...
| 19 | Integrated acceptance game | Reviewable checkpoint commits and evidence are on draft PR55. Verified checkpoints and native+AI restores preserve the campaign. Autonomous gameplay reached55; the new Garrison is independently verified. Verified native+AI restores preserve two bases, completed Formers, citizen allocation and the sovereign plan; checkpoint46 restores on the selector-feedback deployment; the watchdog observation repair is deployed and gated recovery selects the newer automatically verified47 checkpoint. The turn55 Garrison move encountered a native outage; automatic checkpoint recovery succeeded and the capture repair is deployed and one sovereign has resumed from coherent55 recovery. Matched saved-state/action replay passed to56 without reproducing the outage; that operator replay was discarded through coherent recovery. See [evidence](benchmarks/gameplay-turn55-native-outage.json). | Full game is unfinished. Continue monitoring, repair, PR updates and deployment; never merge automatically. |
