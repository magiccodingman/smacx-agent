...
| 4 | Final provider request, runtime envelope and omission audit | Actual Hermes process capture equals controlled receiving provider in four profiles; journal selection inventory and GC metrics | Deployed wire exposed stale saved system on resume; actual two-invocation Hermes regression now verifies replacement plus history continuity. Fresh AI - 4 wire matches approved hash; resumed AI - 4 wire also matches and retains handoff conversation; real Hermes length-continuation now passes exclusive lease enforcement and GC-stable episode identity; longer-run deployed verification remains |
