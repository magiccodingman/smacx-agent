...
| 14 | Tool/runtime failure audit | Pre-MCP rejection capture, typed error metrics, runtime failure events, unmatched provider-request metric | Deployed failures and operational incident export; restricted Hermes checkpoint read permission repaired; one intermittent cold-start cause unresolved |
...
| 16 | Campaign diagnostics download | Scope, compressed/corrupt-tail, immutable-prefix, retention and constrained helper contracts; portal 72/72 including authorization | Deployed archive passes ZIP integrity with journal/history/explicit historical gaps; desktop/390px layout checked. Browser download reports ERR_BLOCKED_BY_CLIENT, so delivery remains unverified; live consistency/overhead pending |
| 17 | Minimal prompt/tool education | Doctrine/schema contracts; exact operational prompt 1,324 Qwen tokens; live prefix reuse evidence | Explicit recompile succeeded in profile but saved Hermes prompt reached wire; request-time canonical replacement now passes real resume test. Verify deployed corrected hash |
