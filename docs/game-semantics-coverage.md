...
| 4 | Final provider request, runtime envelope and omission audit | Actual Hermes process capture equals controlled receiving provider in four profiles; journal selection inventory and GC metrics | Deployed wire exposed stale saved system on resume; actual two-invocation Hermes regression now verifies replacement plus history continuity. Fresh AI - 4 wire matches approved hash; resumed AI - 4 wire also matches and retains handoff conversation; live turn-16 handoff reached provider with contradictory fresh-frame guidance; explicit/automatic response precedence repair and regressions pass, deployment pending; real Hermes length-continuation now passes exclusive lease enforcement and GC-stable episode identity; real receiving-provider tests now verify direct15/9 schemas across resume, direct MCP guard receipt and GC for both namespaces; live turn-16 provider wire now verifies exactly 15 schemas and approved recompiled prompt hash; sustained use remains |
...
| 9 | Ready-unit focus preserves management access | Nonmandatory focus metadata; native management effects with units present; typed review | Live last-ready Auto Explore closed turns before intended management; offered choices now expose existing conservative closure risk, opaque/intent guard contracts pass; live End turn notice reaches actual provider request and sovereign queries production before closure; next automatic-unit closure and sustained behavior remain |
