...
### Turn47 watchdog observation gap

Incident incident-ae8a009faa9e445a92cc289a8fa0dc93 quarantined the worker and collectors after363.6 seconds of purported unchanged gameplay. The retained trace instead contains a successful sovereign production switch from Formers to Synthmetal Garrison at base1334, followed by delivered observation of that effect92 seconds before the incident. The supervisor's compact native snapshot omits base production; its concurrency revision changes but is correctly excluded because revisions also include failed attempts. The repair adds a bounded digest of observed owned base/unit state, independent of receipts and revision churn. Watchdog thresholds remain unchanged. Fingerprint and supervisor contracts pass; native build, controlled effects and deployed recovery remain pending. The quarantined campaign and diagnostic bundle are preserved.

Final owned-digest candidate passes the complete isolated control/MCP/native suite, including actual managed production/citizen digest changes, stable repeated reads, and two coherent native/AI restores. C++ foreign-state/slot-order, native observation, fair-play-world and final doctrine adapter regressions pass. Optional live provider inference and action-containment mode were disabled in this suite; no autonomous proof is claimed from it. Deployment and incident recovery are the next checkpoint.

Owned-state deployment checkpoint: final tested images are deployed, retry-after-update clears incident-ae8a009faa9e445a92cc289a8fa0dc93, and exactly one run-f08943446ad04e72b6bc150854bd05ca resumes. Recovery selected the newer automatically verified checkpoint-b9671b4b35f8488c9c21a861c65df036 at47, not the previously noted46 checkpoint. Timeline-restore-60d862202ff24cd69016ed87 and session-657a47941ec14830966f3d95619e8377 preserve the Spartan encounter. Native owned_progress_digest is present and the supervisor samples its fingerprint. The portal is unpaused. Autonomous same-turn effect reset and full-game acceptance continue.

### Supervisor token accounting checkpoint

Installed Hermes CanonicalUsage includes reasoning in output_tokens; its reasoning_tokens field is a detail bucket. The supervisor added both, inflating spending and potentially tripping its token threshold early. The turn47 incident's output delta is14,856, not28,614 (13,758 reasoning detail tokens were counted twice). This does not explain the missing native effect and does not change the six-minute/call/token thresholds. The repair uses nonnegative output-total delta. Production supervisor regression passes at2,500 output with2,000 reasoning (one call, no token stop) and at4,096 output (stop with exact count); it rejects the old calculation. Existing continuation, real-stall and outage contracts still pass. Images built; checkpointed deployment is next. Gameplay reached49 after the native observation repair.

Accounting deployment passes packaged supervisor regression. Verified checkpoint-b50d5f9785c14343bc9840f5cf942c76 captures50, the updated control/harness images deploy, and run-6687fc8163bf4cd4a42a4221960cc74b resumes with the same native session-657a47941ec14830966f3d95619e8377. No rollback or worker replacement occurred. The portal is unpaused and the next live frame exposes a ready Scout at1575.

### Conditional production flow checkpoint

Independent native reads at47/49/51 show Headquarters Garrison fixed at1/30 minerals, intake2 entirely consumed by support2. The sovereign received net surplus0 but treated queued production as an adequate defensive response. Base mechanics also called its finite ETA unknown without identifying the known conditional accumulation stall. Managed production context and base mechanics now expose no_passive_progress_at_current_surplus when fresh finite inputs show remaining cost and nonpositive surplus. Missing/stale inputs remain unknown; already-funded cost is pending native processing. Doctrine clarifies queued units versus fielded defenders and the zero-surplus condition without choosing an action. Mounted/package adapter, counterfactual and doctrine regressions pass;15 schemas remain7,166 conservative tokens. The campaign is stopped with a fresh verified checkpoint; deployment/native-context verification is next.

Production-flow deployment restores verified52 checkpoint-757b50b2f9d64f9ca50939af203c2a4d into timeline-restore-792b77b1144f4f6881f771b0. Actual request0be058cbd663402d81734584d6fa45fc contains the revised doctrine as the sole system message, exactly matching configured SHA f4d14e8af14ddbd388f2d4e71bfae0e6021fb4b29644ad73cc447ac0fa362504. An operator managed/native query now observes Headquarters3 minerals and+2 net surplus, correctly reported as accumulating; the earlier zero-surplus condition has changed, so no live post-repair zero-case behavior is claimed. One sovereign resumes and reaches53.

Actual movement delivery gate passes at52: sovereign call chatcmpl-tool-8216908776997bad retreats own-unit-12 from1856 to1776. Next actual request43ef811ff3794e2daf92bec61cc86e32 retains the full public receipt, including reported arrival and historical-state qualifiers. The sovereign handoff acknowledges that evidence. Independent native perspective page at53 confirms own-unit-12 tile1776. Future nondisplacement interpretation remains separately unobserved; full game is unfinished.

### Turn213 intent guidance checkpoint

The turn174 wrong-guard write confirms a rejected update, not lost committed cognition. The system contract now makes the final-ready-unit automatic boundary explicit, requires fresh guarded intent writes before it, and explains explicit stale-write retry and current-turn horizons. Strict prompt integrity, canonical intent reconciliation and production memory-guard adapter contracts pass. The isolated strict-prompt test had omitted the current Hermes turn_context module seam; that fixture is repaired. Exact Qwen operational prompt cost grows1336→1470 tokens (+134). Deployment, actual provider delivery and sustained sovereign follow-through are separate pending gates; see `benchmarks/gameplay-intent-guidance-turn213.json`.

Deployment acceptance: control/harness rebuilt, packaged memory guard contract passed, old sovereign stopped before replacement and exactly one new run started. The native session remains unchanged at214. Actual provider request `0d58b82387254ba08ed3aaf4079c050d` contains one system message with final-unit, stale-retry and intent-horizon guidance. Portal supervision and native failure capture remain active. This proves delivery, not behavioral compliance.
...
### Turn214 COUNTER0 capability gap

Native counteroffer dialogue exposes no managed choices; the sovereign waited and reported a capability gap, then supervision preserved the native worker. Bundle SHA and bounded findings are in `benchmarks/gameplay-counteroffer-gap-turn214.json`. The later cognition-provenance deployment replaced MCP through WorkerManager but did not unpause or restore native state. Resume attempts fail closed. Native counteroffer semantics and controlled effect validation are the next repair checkpoint; do not resume merely because services are healthy.

COUNTER0 source checkpoint: stock proposal_menu builds COUNTER0 for ordinary consideration and COUNTER1 for gifts. Its menu construction proves all-research ID7 and conditional threat ID3, correcting the prior misleading cancel_pact7 mapping. COUNTER0 energy returns to native negotiation rather than invoking the gift helper. The compiled production mapping/renderer test passes. An isolated actual stock-menu fixture now covers cancel, goodwill, price request, threat, energy and all-research return IDs with treasury unchanged; execution awaits the worker build. Native downstream negotiated effects remain a separate gate.

COUNTER0 native selector acceptance passes: actual stock proposal_menu returns cancel0/goodwill1/name_price2/threat3/energy5, and COUNTER1 all-research7. Cancellation clears the native proposal; other selections preserve the chosen native proposal ID. Treasury remains500 and stale repeated selections are rejected. This proves stock menu routing/return values, not downstream negotiated deal effects. Full native/MCP recovery regression is running. Isolated resources and the unreferenced prepared image were cleaned.

Counteroffer integrated acceptance: full native/MCP suite exits0, including15-tool enforcement, native gift/purchase/loan/council effects, four production upkeeps, movement completion, journaled intent and two checkpoint restores after timeline GC. Owned identities, plan/conflict/stationary assignment survive; old-session choices reject. Base-site probe227.57ms and counterfactual-site213.048ms. Optional inference was not configured. Deployment and actual sovereign COUNTER0 use remain pending.

Counteroffer deployment acceptance: supported retry-after-update clears the incident and restores verified214 checkpoint,19 owned units,2 bases,799 credits and plan revision14 on the reviewed native build. One sovereign resumes, portal supervision is enabled, native failure capture is armed. Actual first provider request retains plan14 and shares the exact attention lease/episode with the runtime-selection record containing its working journal head and budgets. This proves provenance and delivery for that request, not strategy use. Actual repaired COUNTER0 sovereign selection remains pending. Current and previous prepared builds remain; older unreferenced images were cleaned, leaving123GB free.
...
Actual request324d690e93b74d06aa3b57a6b9566c82 precedes the215 handoff and belongs to a following episode2ccb1f09405ac99b0f398140aa34aee8 and retains restored plan14 with its original turn192 confirmation. It includes the restored native session/timeline, revision17 force composition/production/orders, explicit no-active-task Former counts and nonmandatory ready-unit focus with other-management guidance. Plan-health text explicitly limits assessment to declared bindings and does not verify narrative currency. Plan objective truncation is flagged. The sample proves delivery, not updated intent or competent strategic use; native snapshots and world projection counts remain separately scoped. Evidence and coverage rows3/4 are updated in `benchmarks/gameplay-cognition-omission-audit.json`.

### Turn215 checkpoint storage and containment repair

Checkpoint SQLite VACUUM exceeds the helper tmpfs during repeated portal attempts, despite available host storage. Runtime-context clients time out and Hermes exits zero; clean-yield stopping leaves the match runnable, so portal creates a replacement. Portal supervision is paused and the replacement is stopped. Private volume-backed staging and a bounded inactive native save pair protect checkpoint capacity and the previously verified native half. Clean-yield exhaustion now latches the match and quarantines native/collectors. Actual large-helper/WAL/memory and manager failure-injection contracts pass. Deployment and final-image native recovery remain pending; no repaired live success is claimed. Evidence: `benchmarks/gameplay-checkpoint-storage-turn215.json`. The earlier request324d sample is explicitly pre-handoff, not a successful turn215 resumed provider request.

Standalone clean-yield recovery also now clears its own incident only after native/AI recovery and collectors succeed. Eight controlled orchestration cases cover legacy and staged slot metadata in solo/LAN, including failed identity imports that remain latched. This correction follows the initial integration image; final-image acceptance remains pending.

The client-side runtime fetch now records a normalized, episode-linked failure before raising; provider interception alone cannot capture this pre-submission failure. Timeout/HTTP/transport cases retain fail-closed behavior and exclude endpoint credentials, and summary/metrics consumption passes. Deployment remains pending.

Checkpoint-repair native integration passes (exit0):15 managed tools, native effects, crash recovery, repeated same-checkpoint restores after timeline GC and preserved typed intent. Counterfactual site probe gap231.57ms. This run predates the post-success incident-clearance/client-failure trace additions. Installed final-harness failure capture passes four cases; final-image native integration is running separately, and deployment remains pending.

The recovery preflight now compares the actual native file SHA256 and byte count before restoring any AI memory. Previously only the recorded digest format was checked. Twelve controlled legacy/staged solo/LAN cases pass, including mismatch rejection before journal/Hermes restoration or worker startup. This final guard follows the image currently under integration test and is not deployed yet.

The incident-clearance/client-trace image also passes full native/MCP integration (exit0), including repeated recovery and222.53ms counterfactual probe gap. The final native-file digest-preflight image has a separate integration run in progress; it remains the deployment gate.

Digest preflight integration failed closed because parking compresses the native save before the new preflight. The helper now hashes decoded `.sav.zst` bytes without extraction, rejects corrupt archives, and retention protects both internal checkpoint slots. Plain/compressed identity and packaged worker retention tests pass; the corrected full native/MCP suite is running. Production stays paused at215; this checkpoint is not deployed.
...
Turn216 reopens request-context acceptance after successful automatic checkpoint792d245b. New client diagnostics prove a pagination revision race returned409 and another fresh context completed about1.6 seconds after the ten-second client deadline. Both terminal action receipts are absent from their immediate next provider requests after episode restart. Both revision-race receipts now retry before attention; a bounded120-second context wait preserves the pending history during valid longer collection. Actual HTTP retry/delay, failure and prompt contracts pass; packaged validation/deployment remain pending. Production is held with zero sovereign runs. [Evidence](benchmarks/gameplay-context-cut-turn216.json).

Context-cut packaged acceptance passes: installed control handler retries only allowed revision races and preserves restart fencing; installed harness receives10.509-second real HTTP context without losing pending receipt or changing episode. Full runtime contract passes64K/256K and Huge chaotic25998-token bounds. Deployment remains the next gate.

Context-cut deployment at216 passes supported checkpoint/recovery with18 units,2 bases and819 credits. Actual request5dc1ba43 in episoded289d49b includes restored plan revision15 and its216 confirmation. One sovereign run9c820b61, portal supervision and native failure capture are active. Monitoring now explicitly includes pre-provider context failures. Next live boundary and full-game acceptance remain open; see `benchmarks/gameplay-context-cut-turn216.json`.
