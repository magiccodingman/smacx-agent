...
| Spectating | non-participant admin observation, opt-in authenticated observer deck, permanent campaign-participant exclusion, seat switching, worker-enforced read-only mode | real stream plus authorization/read-only checks |
...
| Provider management | OpenAI-compatible discovery, keyed/unkeyed providers, chosen model/context, generic sampling controls, typed custom request parameters, import/export, and honest accepted/rejected/stale request probes | contract tests, browser form QA, and real Qwen endpoint |
| AI profiles | uniquely named editable model/reasoning/context/notes identities, inactive filtering, reversible deactivation, exact editable templates, provider-neutral custom settings, and per-seat personality selection | .NET/Control contracts and real managed run |
...
| Analytics | scoped history, turn duration, Hermes input/output/cache/reasoning/API counters, CSV, isolated read-only SQL lab; content-free purpose-aware embedding calls/tokens/latency/throughput/errors and numerical/semantic canary history | fresh-schema .NET tests, real Hermes telemetry query, and live local-embedding observatory |
